...
static ssize_t
thermal_message_of_batt_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	if (!tm || !tm->message_ok)
		return -EINVAL;
	...
	return snprintf(buf, PAGE_SIZE,
			"array_size %s\nscreen_on %s\nscreen_off %s\n",
			tm->batt_array_size, tm->batt_level_screen_on,
...
#ifdef CONFIG_DRM
static ssize_t
thermal_screen_state_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", sm.screen_state);
}
...
static ssize_t
thermal_sconfig_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
...
static ssize_t
thermal_sconfig_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t
thermal_boost_store(struct device *dev, struct device_attribute *attr,
			const char *buf, size_t len)
{
	int ret;
...
static ssize_t
thermal_temp_state_show(struct device *dev, struct device_attribute *attr,
			char *buf)
{
...
static ssize_t
thermal_temp_state_store(struct device *dev, struct device_attribute *attr,
			const char *buf, size_t len)
{
	int val = -1;
