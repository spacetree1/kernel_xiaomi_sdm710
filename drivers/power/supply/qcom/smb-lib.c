...
static int smblib_get_jeita_cc_delta(struct smb_charger *chg, int *cc_delta_ua)
{
	...
	if ((stat & BAT_TEMP_STATUS_HOT_SOFT_LIMIT_BIT) ==
		...
		if (cc_minus_ua != chg->jeita_ccomp_hot_delta) {
			...
			if (rc < 0)
				smblib_err(chg,
					"Couldn't configure jeita_cc rc=%d\n",
					rc);
		}
...
static int smblib_enable_otg_wa(struct smb_charger *chg)
{
	...
	for (i = 0; i < ARRAY_SIZE(otg_current); i++) {
		...
		if (retry_count >= MAX_RETRY) {
			...
			/*
			 * if hiccup is disabled, when otg enable ok,
			 * should enable hiccup
			 */
			...
			if (stat == 0x0f) {
				rc = smblib_write(chg, OTG_ENG_HICCUP_MODE,
							0x00);
				if (rc < 0)
					smblib_err(chg,
						"Couldn't configure OTG_ENG_HICCUP_MODE rc=%d\n",
...
static void check_usb_status(struct smb_charger *chg)
{
	 ...
	 */
	if (usb_present == 1 && !off_charge_flag && vbat_uv <= CUTOFF_VOL_THR) {
...
static int smblib_therm_charging(struct smb_charger *chg)
{
	...
	case POWER_SUPPLY_TYPE_USB_HVDCP_3:
		thermal_icl_ua = chg->thermal_mitigation_qc3[
							chg->system_temp_level];
		break;
	...
	if (chg->system_temp_level == 0) {
		...
		smblib_dbg(chg, PR_MISC,
			"thermal_icl_ua is %d, chg->system_temp_level: %d\n",
			       ...
			       union power_supply_propval *val)
{
	 ...
	 */
	if (off_charge_flag && rc == 0 && usb_present == 1
...
static void smblib_check_vbus_work(struct work_struct *work)
{
	...
	if (usb_present
		...
		if (val.intval < QC2_HVDCP_VOL_UV_THR) {
			...
			smblib_set_usb_pd_allowed_voltage(chg, MICRO_5V,
								MICRO_5V);
	...
	if (vbus_rising) {
		...
		if (chg->wa_flags & BOOST_BACK_WA) {
			...
			if (data) {
				...
				vote(chg->usb_icl_votable, CC_FLOAT_VOTER,
						false, 0);
				vote(chg->usb_icl_votable, WEAK_CHARGER_VOTER,
						false, 0);
...
static void smblib_handle_apsd_done(struct smb_charger *chg, bool rising)
{
	...
	if (is_client_vote_enabled(chg->usb_icl_votable, CC_FLOAT_VOTER)) {
	...
	case FLOAT_CHARGER_BIT:
		...
		/*
		 * if floated charger is detected, and audio accessory
		 * set icl to 500
		 */
