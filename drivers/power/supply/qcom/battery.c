...
#ifdef CONFIG_CHARGER_BQ25910_SLAVE
static ssize_t const_slave_pct_show(struct class *c,
...
static ssize_t const_slave_pct_store(struct class *c,
			struct class_attribute *attr, const char *ubuf,
			size_t count)
{
	struct pl_data *chip = container_of(c, struct pl_data, qcom_batt_class);
	unsigned long val;
...
static void handle_main_charge_type(struct pl_data *chip)
{
	...
	if (!get_effective_result_locked(chip->pl_disable_votable)) {
		if (taper_soc > TAPER_CAPACITY_THR
			...
			vote(chip->pl_disable_votable, PL_HIGH_CAPACITY_VOTER,
				true, 0);
			return;
		}
	} else {
